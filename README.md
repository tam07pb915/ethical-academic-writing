...
> **注意**：スキルはdescriptionに基づいてClaudeが自動判断で読み込みます。スラッシュコマンドによる明示的な呼び出しはclaude.aiではできません（Claude Codeのみ対応）。
...
Claude Codeではスキルが自動読み込みされるほか，`/ethical-academic-writing` のようにスラッシュコマンドで明示的に呼び出すこともできます。

### Project Instructionsに貼る方法（非推奨）

SKILL.md の内容をProject Instructionsに貼り，referenceファイルをProject Knowledgeとしてアップロードする方法もありますが，referenceファイルがClaudeに参照されないことがあるため，**カスタムスキルとしてのアップロードを推奨**します。
